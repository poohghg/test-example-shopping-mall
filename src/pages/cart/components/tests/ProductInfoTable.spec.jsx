...
import { cartValidationMessages } from '@/messages.js';
...
it('장바구니에 포함된 아이템들의 이름, 수량, 합계가 제대로 노출된다', async () => {
  ...
  expect(within(firstItem).getAllByRole('textbox')).toHaveValue('3');
  ...
  expect(within(secondItem).getAllByRole('textbox')).toHaveValue('4');
...
it('특정 아이템의 수량이 변경되었을 때 값이 재계산되어 올바르게 업데이트 된다', async () => {
  ...
  expect(within(firstItem).getByText(formatPrice('4045'))).toBeInTheDocument();
});

it('특정 아이템의 수량이 1000개로 변경될 경우 "최대 999개 까지 가능합니다!"라고 경고 문구가 노출된다', async () => {
  const alertSpy = vi.fn();
  // window.alert -> alertSpy로 대체한다.
  ...
  expect(alertSpy).toHaveBeenNthCalledWith(
    1,
    cartValidationMessages.MAX_INPUT_VALUE,
  );
});
...
it('특정 아이템의 삭제 버튼을 클릭할 경우 해당 아이템이 사라진다', async () => {
  ...
  // queryBy~ 는 요소의 존재유무를 판단. 존재 하지 않아도 에러 x
});
